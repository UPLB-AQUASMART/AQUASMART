...
function initialWell(): Well {
  return { id: 1, name: "Option 1", discharge: 138, x: 51, y: 53, readings: makeReadings(1, 138) };
}
...
export default function GroundwaterSimulationPage() {
  ...
  const scenario = useMemo(() => {
    ...
    const averageDrawdown = drawdowns.reduce((sum, value) => sum + value, 0) / Math.max(wells.length, 1);
    ...
    return {
      ...
      criticalWells,
      sustainability,
      recoveryTime: Math.round(36 + averageDrawdown * 22 + capacityUtilization * 0.28 + criticalWells * 18),
    };
  }, [totalDischarge, wells]);

  const averages = useMemo(() => {
    const average = (key: ReadingKey) => wells.reduce((sum, well) => sum + well.readings[key], 0) / Math.max(wells.length, 1);
    return {
  ...
  const donutData = useMemo(() => ({
    ...
    datasets: [{
      data: [averages.dissolvedOxygen, averages.ph, averages.temperature, averages.salinity * 10, averages.tds / 100, averages.electricalConductivity / 100, averages.groundwaterLevel],
      backgroundColor: ["#0b1f3a", "#46c5df", "#4dbb5d", "#ee9b22", "#638de3", "#2f8f63", "#09a9d5"],
      borderColor: "#fff",
      borderWidth: 2,
      hoverOffset: 7,
    }],
    ...
    datasets: [
      {
        ...
        data: [5.2, 6.2, 7.2, 4.2, 3.2, 5.2, 2.2, 4.2, 6.2, 1.2, 3.2, 5.2, 7.2, 6.2].map((offset) => averages.temperature + offset),
        borderColor: "#0b2545",
      ...
      },
      {
        ...
        data: [-2.8, -1.8, -0.8, -3.8, -4.8, -2.8, -5.8, -3.8, -1.8, -6.8, -4.8, -2.8, -0.8, -1.8].map((offset) => averages.temperature + offset),
        borderColor: "#46c5df",
  ...
  function updateWell(id: number, updates: Partial<Well>) {
    setWells((current) => current.map((well) => well.id === id ? { ...well, ...updates } : well));
  }
  ...
  function moveWell(event: ReactPointerEvent<HTMLDivElement>, id: number) {
    ...
    if (cardBounds) {
      ...
      if (insideProtectedArea) {
        const distanceToTop = Math.abs(pointerY - (cardBounds.top - wellClearance));
        const distanceToRight = Math.abs(pointerX - (cardBounds.right + wellClearance));
        if (distanceToTop <= distanceToRight) pointerY = cardBounds.top - wellClearance;
        else pointerX = cardBounds.right + wellClearance;
      }
    }

    const x = Math.min(94, Math.max(6, ((pointerX - bounds.left) / bounds.width) * 100));
    const y = Math.min(88, Math.max(15, ((pointerY - bounds.top) / bounds.height) * 100));
  ...
  function downloadPdfReport() {
    const headers = ["Well ID", "Discharge", "DO", "pH", "Temp", "Salinity", "TDS", "EC", "GW Level", "Map X", "Map Y"];
    ...
    headers.forEach((header, index) => {
      lines.push(`BT /F1 7 Tf ${x + 4} ${y + 8} Td (${normalizePdfText(header)}) Tj ET`);
    ...
    rows.forEach((row) => {
      ...
      row.forEach((cell, index) => {
        lines.push(`BT /F1 7 Tf ${x + 4} ${y + 8} Td (${normalizePdfText(cell)}) Tj ET`);
        if (index > 0) lines.push(`${x} ${y} m ${x} ${y + rowHeight} l S`);
        x += widths[index];
      });
      y -= rowHeight;
    });
    lines.push("BT /F1 8 Tf 32 30 Td (Generated by AQUASMART Mini. Upload this report to restore its well configuration.) Tj ET");
  ...
  async function uploadPdfReport(file?: File) {
    if (!file) return;
    try {
      const cells = extractPdfCells(new TextDecoder("latin1").decode(await file.arrayBuffer()));
      const start = cells.findIndex((cell, index) => cell === "Well ID" && cells[index + 1] === "Discharge" && cells[index + 2] === "DO");
      ...
      setSelectedWellId(null);
      setImportMessage(`Imported ${imported.length} well${imported.length === 1 ? "" : "s"} from PDF.`);
    } catch {
      setImportMessage("This PDF could not be imported. Please use a report exported from this page.");
    }
  }
    ...
    <main className={styles.page}>
        ...
        <div className={styles.heroContent}>
          ...
          {!simulationOpen ? (
            <div className={styles.heroBottom}>
              <p>Simulate groundwater response across wells and field zones using MODFLOW/FLOPY outputs, proximity layers, and live parameter readings from AQUASMART mini.</p>
              <button className={styles.eyeButton} type="button" onClick={() => setSimulationOpen(true)} aria-label="Open groundwater simulation">
          ...
          {(simulationOpen ? wells : wells.slice(0, 1)).map((well) => (
            ...
            >
              ...
              <button className={`${styles.wellMarker} ${selectedWellId === well.id ? styles.selected : ""}`} type="button" onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedWellId((current) => current === well.id ? null : well.id); }} aria-label={`Configure ${well.name}`}>
          ...
          {simulationOpen && selectedWell && (
            <div className={styles.wellPopover} style={{ left: `${Math.min(72, selectedWell.x + 7)}%`, top: `${Math.max(5, selectedWell.y - 28)}%` }}>
              <button className={styles.closePopover} onClick={() => setSelectedWellId(null)} aria-label="Close well controls"><X size={15} /></button>
              <div className={styles.popoverTitle}>
                <label className={styles.wellNameEditor}>
                  <Droplet size={18} />
                  <input value={selectedWell.name} onChange={(event) => updateWell(selectedWell.id, { name: event.target.value })} aria-label="Well name" title="Edit well name" maxLength={32} />
                  <Pencil size={13} aria-hidden="true" />
                </label>
                <strong>{selectedWell.discharge} m³/day</strong>
              </div>
              <input className={styles.range} type="range" min="0" max="500" value={selectedWell.discharge} onChange={(event) => updateDischarge(selectedWell.id, Number(event.target.value))} aria-label={`${selectedWell.name} discharge`} />
              <div className={styles.rangeLabels}><span>0</span><span>500</span></div>
      ...
      <section className={styles.readings} aria-labelledby="readings-title">
        <div className={styles.readingsHeader}>
          <h2 id="readings-title">All Parameters — Current Readings (All Zones)</h2>
          <div className={styles.reportActions}>
            <label><Upload size={16} /> Upload PDF<input type="file" accept="application/pdf" onChange={(event) => { void uploadPdfReport(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label>
            <button onClick={downloadPdfReport}><Download size={16} /> Download PDF</button>
          </div>
        </div>
        ...
        <div className={styles.tableWrap}>
          <table>
            ...
            <tbody>{wells.map((well) => (
              <tr key={well.id}>
                <td><input className={styles.nameInput} value={well.name} onChange={(event) => updateWell(well.id, { name: event.target.value })} aria-label={`${well.name} name`} /></td>
                <td><input type="number" min="0" max="500" value={well.discharge} onChange={(event) => updateDischarge(well.id, Number(event.target.value))} /></td>
                <td><input type="number" step="0.1" value={well.readings.dissolvedOxygen} onChange={(event) => updateReading(well.id, "dissolvedOxygen", Number(event.target.value))} /></td>
                <td><input type="number" step="0.1" value={well.readings.ph} onChange={(event) => updateReading(well.id, "ph", Number(event.target.value))} /></td>
                <td><input type="number" step="0.1" value={well.readings.temperature} onChange={(event) => updateReading(well.id, "temperature", Number(event.target.value))} /></td>
                <td><input type="number" step="0.1" value={well.readings.salinity} onChange={(event) => updateReading(well.id, "salinity", Number(event.target.value))} /></td>
                <td><input type="number" value={well.readings.tds} onChange={(event) => updateReading(well.id, "tds", Number(event.target.value))} /></td>
                <td><input type="number" value={well.readings.electricalConductivity} onChange={(event) => updateReading(well.id, "electricalConductivity", Number(event.target.value))} /></td>
                <td><input type="number" step="0.1" value={well.readings.groundwaterLevel} onChange={(event) => updateReading(well.id, "groundwaterLevel", Number(event.target.value))} /></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
        ...
        <div className={styles.analytics}>
          <article className={styles.chartCard}>
            <div className={styles.chartHeading}><h3>Average <span>Water Parameters</span></h3><SlidersHorizontal size={16} /></div>
            <div className={styles.donut}><Doughnut data={donutData} options={{ responsive: true, maintainAspectRatio: false, cutout: "62%", plugins: { legend: { position: "bottom", labels: { boxWidth: 11, color: "#0b2545", padding: 14 } } } }} /></div>
          </article>
          <article className={styles.chartCard}>
            <div className={styles.chartHeading}><h3><Thermometer size={19} /> Temperature Trends</h3><SlidersHorizontal size={16} /></div>
            <div className={styles.line}><Line data={lineData} options={{ responsive: true, maintainAspectRatio: false, interaction: { mode: "index", intersect: false }, plugins: { legend: { position: "top" } }, scales: { y: { title: { display: true, text: "Temperature (°C)" } } } }} /></div>
          </article>
        </div>
      </section>

      <section className={styles.research}>
        <div className={styles.researchHeading}><p>Research Institutions/Partners</p></div>
        <div className={styles.researchGrid}>{researchCards.map((image, index) => (
          <article className={styles.researchCard} key={image}><img src={image} alt="" /><div><h3>Partner Research {index + 1}</h3><p>Groundwater data, field observations, and irrigation planning references.</p></div></article>
        ))}</div>
      </section>

      <footer className={styles.footer}>
        <div className={styles.footerBrand}><img src="/figma/groundwater-footer-mark.png" alt="" /><div><p>AQUASMART mini</p><span>Groundwater simulation and water-quality monitoring for field decisions.</span></div></div>
        <div className={styles.footerLinks}>{navLinks.map((link) => <Link href={link.href} key={link.label}>{link.label}</Link>)}</div>
      </footer>
    </main>
  );
}
