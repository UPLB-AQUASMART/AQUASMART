...
void main() {
  ...
  float current = sin((p.x * 0.75 - p.y * 1.25 + field * 2.7 + t * 0.8) * 3.2);
  float softCurrent = smoothstep(-0.85, 0.85, current);
  float deepWater = smoothstep(0.43, 0.78, field + warpB.x * 0.15);
  float freshWater = smoothstep(0.34, 0.72, warpA.y + softCurrent * 0.14);
  ...
  vec3 color = mix(pale, mist, smoothstep(0.2, 0.82, field));
  color = mix(color, aqua, deepWater * 0.88);
  color = mix(color, navy, smoothstep(0.63, 0.92, field + softCurrent * 0.16) * 0.78);
  color = mix(color, green, freshWater * (1.0 - deepWater) * 0.38);

  float highlight = smoothstep(0.68, 0.92, warpB.y + softCurrent * 0.12);
  color = mix(color, vec3(1.0), highlight * 0.38);

  float grain = hash(gl_FragCoord.xy + floor(u_time * 24.0));
