...
export function PrecipitationSection({
  ...
  const safeSelectedHourIndex = Math.max(
    0,
    Math.min(selectedHourIndex, selectedDay.hourlyRain.length - 1),
  );
  const selectedHourPeriodLabel = hourlyPeriodLabel(safeSelectedHourIndex);
  const rainfallMm = roundHourlyRain(
    selectedDay.hourlyRain[safeSelectedHourIndex] ?? 0,
  );
  const rainfallLiters = rainfallVolumeLiters(rainfallMm);
  const isDry = rainfallMm <= 0;
  const rainIntensity = isDry
    ? 0
    : Math.min(Math.max(rainfallMm / maxHourlyRain, 0.08), 1);
  const waterLevelPercent = isDry
    ? 0
    : Math.min(Math.max((rainfallMm / maxHourlyRain) * 100, 8), 100);
  ...
  const activeViewLabel =
    rainfallView === "2d"
      ? "Rainfall depth equivalent view"
      : "3D rainfall equivalent field model";
        ...
        <article className={styles.visualCard}>
          <div className={styles.cardHeader}>
            ...
            <div
              ...
              data-view={rainfallView}
              role="group"
              aria-label="Rainfall visualization view"
            >
              <button
                type="button"
          ...
          <div
            className={styles.rainfallModel}
            key={`${selectedDay.date}-${rainfallView}`}
          >
            <div className={styles.modelReadout}>
              <span>{activeViewLabel}</span>
              <strong>{rainfallMm} mm</strong>
              <small>
                Projected rainfall from {selectedHourPeriodLabel} over a 1
                hectare crop field. Daily total: {selectedDay.rain} mm.
              </small>
            </div>

            <div
              className={`${styles.modelStage} ${rainfallView === "3d" ? styles.modelStage3d : ""}`}
            >
              {rainfallView === "2d" ? (
                <Rainfall2DModel
          ...
          <div className={styles.dayPicker}>
            {days.map((item, index) => (
              <button
                ...
                className={
                  index === safeSelectedDayIndex ? styles.activeDay : undefined
                }
                onClick={() => {
          ...
          <div className={styles.projectionTransition} key={selectedDay.date}>
            <div className={styles.barChart}>
              ...
              {selectedDay.hourlyRain.map((value, index) => (
                ...
                >
                  ...
                  <span
                    style={{
                      height: `${value > 0 ? Math.max((value / maxHourlyRain) * 100, 8) : 3}%`,
                    }}
                  />
                </div>
              ))}
            ...
            <div className={styles.xAxisLabels} aria-label="Hours of the day">
              {hourlyLabels.map((hour, index) => (
                <span
                  key={hour}
                  className={
                    index === safeSelectedHourIndex
                      ? styles.activeHourLabel
                      : undefined
                  }
                >
                  {hour}
            ...
            <div className={styles.timeSlider}>
              <label htmlFor="rainfall-time-slider">
                ...
                <strong>
                  {selectedDay.date}, {selectedHourPeriodLabel}
                </strong>
              </label>
              <input
            ...
            <div className={styles.note}>
              ...
              Showing hourly rainfall for {selectedDay.day}, {selectedDay.date}.
              Total: {selectedDay.rain} mm.
            </div>
          </div>
        </article>
      </div>
