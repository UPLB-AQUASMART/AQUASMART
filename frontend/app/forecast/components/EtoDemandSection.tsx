...
function WeatherConditionIcon({
  ...
  if (normalized.includes("thunder")) {
    return (
      <CloudLightning aria-hidden="true" className={className} size={size} />
    );
  }
  if (
    normalized.includes("rain") ||
  ...
  if (normalized.includes("fog"))
    return <CloudFog aria-hidden="true" className={className} size={size} />;
  if (normalized.includes("cloud") || normalized.includes("overcast")) {
...
export function EtoDemandSection({ data }: EtoDemandSectionProps) {
  ...
  const metrics = etoMetrics.map((metric) => ({
    ...metric,
    value:
      data?.metrics.find((item) => item.label === metric.label)?.value ??
      metric.value,
  }));
      ...
      <div className={styles.layout}>
        <aside className={styles.recommendationCard}>
          ...
          <div className={styles.outlook}>
            ...
            <p>
              {data?.outlook ??
                "Low rain and moderate ETO suggest monitoring soil moisture before irrigation."}
            </p>
          </div>
